...
while(main <2):
    ...
    next_page=driver.find_elements(By.XPATH,"//a[contains(@class,'_833p2h l1j9v1wn')]")[main]
...
c.execute('''CREATE TABLE listings
             (Main_name text, Second_name text, Date text, No_of_beds int, Price real, Service_fee real, Total_before_taxes real, Amenities real, Cleanliness real, Communication real, Accuracy real, Location real, Check_in real)''')

# Insert the data into the table
for i in range(len(main_name)):
    row_data = (main_name_l[i], second_name_l[i], date_l[i], no_of_beds_l[i], price_l[i], Service_fee_l[i], total_bef_taxes_l[i], ", ".join(amenities_l[i]), clealiness_l[i], communication_l[i], accuracy_l[i], location_l[i], check_in_l[i])
    c.execute("INSERT INTO listings VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", row_data)
